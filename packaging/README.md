...
* The draft release notes are generated by `packaging/generate_release_notes.py`:
  an overview of everything that changed since the previous release tag, then
  one entry per commit (what changed + effect). Review and trim the draft
  before publishing.
...
- [ ] Upload Windows .7z chunks to GitHub Draft Release (the CI release workflow does this automatically)
